...
for i in  $(awk -F "\t" '{if(NR>1){print $1"_"$2"_"$3}}' $MANIFEST );do
	...
	if [[ -f ${STEP2_WORKING_FLAG} || -f ${STEP2_DONE_FLAG} ]];then
		...
		CMD="sbatch -J step2_circlator_${HGAP_ANALYSISID} -p cgrq --cpus-per-task 12 --time 196:00:00 -e ${LOG_DIR}/step2_circlator_${HGAP_ANALYSISID}.stderr -o ${LOG_DIR}/step2_circlator_${HGAP_ANALYSISID}.stdout step2_running_hgap_single.sh $i $HGAP_ANALYSISID"
