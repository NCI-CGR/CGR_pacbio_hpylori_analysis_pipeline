...
for i in  $(awk -F "\t" '{if(NR>1){print $1"_"$2"_"$3}}' $MANIFEST);do
	...
	if [[ -f ${STEP3_WORKING_FLAG} || -f ${STEP3_DONE_FLAG} ]];then
		...
		CMD="qsub -cwd -q $QUEUE -N Step3_format_hgap_${HGAP_ANALYSISID} -e ${LOG_DIR}/step3_format_hgap_${HGAP_ANALYSISID}.stderr -o ${LOG_DIR}/step3_format_hgap_${HGAP_ANALYSISID}.stdout -S /bin/sh step3_format_hgap_result.sh ${ROOT_DIR}/${i}/HGAP_run/circlator_${HGAP_ANALYSISID}/05.clean.fasta"
