...
for i in  $(awk -F "\t" '{if(NR>1){print $1"_"$2"_"$3}}' $MANIFEST);do
	...
	CGR_ID=$(echo $i | awk -F"_" '{print $(NF-1)}')
	...
	if [[ -f ${STEP3_WORKING_FLAG} ]] || [[ -f ${STEP3_DONE_FLAG} ]];then
		...
		CMD="sbatch --time 96:00:00 -e ${LOG_DIR}/step3_format_hgap_${HGAP_ANALYSISID}.stderr -o ${LOG_DIR}/step3_format_hgap_${HGAP_ANALYSISID}.stdout step3_format_hgap_result_V2.sh ${ROOT_DIR}/${i}/HGAP_run/circlator_${HGAP_ANALYSISID}/05.clean.fasta"
		echo $CMD >>  NOHUPS/nohup_step3_$(date +\%Y\%m\%d).txt
		echo $CMD
		eval $CMD

	fi
	rm ${LOG_DIR}/step3_format_hifiasm.working
	if [[ -f ${LOG_DIR}/step3_format_hifiasm.working ]] || [[ -f ${LOG_DIR}/step3_format_hifiasm.done ]];then
		...
		#CMD="qsub -cwd -q $QUEUE -N Step3_format_hifiasm_${HGAP_ANALYSISID} -e ${LOG_DIR}/step3_format_hifiasm_${HGAP_ANALYSISID}.stderr -o ${LOG_DIR}/step3_format_hifiasm_${HGAP_ANALYSISID}.stdout -S /bin/sh step3_format_hgap_result_hifiasm.sh ${ROOT_DIR}/${i}/hifiasm_run/circlator/05.clean.fasta"
		CMD="sbatch --time 96:00:00 -e ${LOG_DIR}/step3_format_hifiasm_${HGAP_ANALYSISID}.stderr -o ${LOG_DIR}/step3_format_hifiasm_${HGAP_ANALYSISID}.stdout step3_format_hgap_result_hifiasm.sh ${ROOT_DIR}/${i}/hifiasm_run/circlator/05.clean.fasta"
		echo $CMD >>  NOHUPS/nohup_step3_$(date +\%Y\%m\%d).txt
		echo $CMD
		eval $CMD
