...
echo ${PACBIO_JOB_DIR8}/${HGAP_ANALYSISID_PART1}/${HGAP_ANALYSISID_PART2}/cromwell-job
OUT_HGAP=${ROOT_DIR}/${VIAL_ID}_${CGR_ID}_${HGAP_ANALYSISID}/Delivery/Assembly
OUT_BASEMOD=${ROOT_DIR}/${VIAL_ID}_${CGR_ID}_${HGAP_ANALYSISID}/BASEMOD_hifi_run
...
assemblyReport=$(find ${REAL_JOB_DIR} -name "polished_assembly.report.json" | tail -1)
HEADER=""
CONTENT=""
for i in "Polished Contigs" "Maximum Contig Length" "N50 Contig Length" "Sum of Contig Lengths";do 
	VALUE=$(grep -A 3 "$i" ${assemblyReport} |  head -4 | tail -1 | sed 's/,//g')
	HEADER="$HEADER\t$i" 
	CONTENT="$CONTENT\t$VALUE"
done

for i in ${OUT_HGAP}/*_chromosomal*.fasta;do
...
HAS_PLASMID=$(ls -l ${OUT_HGAP}/*_plasmid*.fasta 2>/dev/null | wc -l)

if [[ $HAS_PLASMID -gt 0 ]];then 
	for i in ${OUT_HGAP}/*_plasmid*.fasta;do 
		CHR_NAME=$(basename $i .fasta)
...
covReport=$(find ${REAL_JOB_DIR} -name "coverage.report.json" | tail -1)
HEADER="Mean Coverage"
CONTENT=$(grep -A 1 "Mean Coverage" ${covReport} | tail -1 | cut -f2 -d: | bc -l | xargs -I {} printf "%5.4f" {})
...
HEADER=""
CONTENT=""
for i in "Number of Polymerase Reads " "Polymerase Read Length Mean " "Polymerase Read N50 " "Number of Subread Bases " "Alignment Length Mean " ;do 
...
BASEMOD_HTML_FOLDER=$(find ${REAL_JOB_DIR}/ -name "kinetic_detections.png" | tail -1 | xargs dirname)
motifHist=$(find ${REAL_JOB_DIR}/ -name "motif_histogram.png" | tail -1)
python tsv2html_colorless.py ${REPORT_TMP}/mapping_stats_report.txt ${REPORT_TMP}/mapping_stats_report_html.txt 
cp ${BASEMOD_HTML_FOLDER}/kinetic_detections.png ${REPORT_DIR}
cp ${BASEMOD_HTML_FOLDER}/kinetic_histogram.png ${REPORT_DIR}
cp ${motifHist} ${REPORT_DIR}
...
for i in ${REPORT_DIR}/coverage_plot_*.png;do 
	IMAGE_NAME=$(basename $i .png | sed 's/coverage_plot_//g')
	echo "<br>" >> $HTML_REPORT
	echo "<h2>coverage_plot_${IMAGE_NAME}</h2>" >> $HTML_REPORT
	echo "<img src=\"coverage_plot_${IMAGE_NAME}.png\" width=\"1000\" height=\"700\" alt=\"${IMAGE_NAME}\">" >> $HTML_REPORT
done
echo "<br>" >> $HTML_REPORT
...
echo "<br>" >> $HTML_REPORT
echo "<img src=\"motif_histogram.png\" width=\"1000\" height=\"700\" alt=\"kinetic_histogram\">" >> $HTML_REPORT
echo "<br>" >> $HTML_REPORT
